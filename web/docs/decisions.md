...
**Directory import: DataForSEO for discovery, the business's own website for details, Google only on demand.** New runs discover businesses with DataForSEO Business Listings Search (about $0.012 per request plus $0.00036 per record at the reference rate) instead of Google Places grid search. Details come from each business's own website over plain HTTP, with evidence kept per field. Google Places is an optional, attributed staff view with explicit field masks and daily and monthly caps; its content is never copied into listings, and only the place id is kept permanently. Every paid call is reserved against a budget before it is sent, and a call with an unknown outcome waits for a person instead of being resent. The proxy, social-page reading and web-search email lookup from 2026-09-24 are removed. DataForSEO ratings and photos and website logos are not published until their terms are confirmed.

**Imported listings: website rules, services and images.** A listing's website is the business's own site or its own social profile, never a directory, map, health-fund page or other third party; booking and WhatsApp links go to their own fields, and a site that shows another business's phone and name is dropped. Services are read from the business's site with or without prices and mapped to our categories. The logo and photos come from the business's own site, are chosen by staff in review and copied to our storage on approval (setting `useWebsiteImages`, on). The unclaimed listing page now shows those photos, the logo, services and prices and the website link, still under the "not managed by the business" notice and without contact buttons.

**Imported listings: a phone or an email is enough, Google rating and profile are used.** A record is ready to publish with a phone or an email (email is no longer mandatory). The Google rating and review count are published with a link to the business's Google profile, which also stands in as the website when the business has none of its own. The Google profile logo and main photo are used when the business's site has none. All three can be switched off in the import settings.

**Listing URLs include the category; unclaimed listings use the full profile template.** A listing's address is `/:region/:category/:slug` (its first category in catalog order); `/:region/biz/:slug` redirects there permanently. Listings no owner has claimed use the same profile template as claimed ones, with a notice that the details come from public sources, no "verified" badge, no enquiry form and no online booking (both need an owner); visitors reach the business by phone, WhatsApp, website or Waze. Imported listings are created with online booking off.

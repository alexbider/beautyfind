...
- Open with the company's accountant: whether Israfind Group registers for Israeli VAT. The site currently promises a חשבונית מס for subscriptions (Get Listed, Billing, Terms). A foreign company without Israeli VAT registration cannot issue one; if it won't register, that copy must change to "invoice" and VAT handling to the business's self-assessment.

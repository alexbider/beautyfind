...
export function MenuEditor({
  ...
  const addRow = () => {
    ...
    setConfirmKey(null);
    touch();
    requestAnimationFrame(() => document.getElementById(`menu-name-${key}`)?.focus());
  };
  ...
  const focusFirstBad = (byKey: Record<string, RowErrors>) => {
    ...
    requestAnimationFrame(() => {
      const el = listRef.current?.querySelector<HTMLElement>(`[data-row="${first.key}"] [aria-invalid="true"]`);
      el?.focus();
  ...
  return (
    <section aria-labelledby="h-menu" className={s.section}>
      ...
      <div className={s.table} ref={listRef}>
              ...
              {open && (
                <div id={editorId} className={s.editor}>
                  ...
                  <div className={s.editActions}>
                    <button type="button" onClick={() => setOpenKey(null)} className={s.outlineBtn}>סגירה</button>
                    {confirmKey === r.key ? (

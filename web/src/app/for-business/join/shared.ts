...
export const STEPS = [
  { key: 'basics', name: 'פרטי העסק', title: 'נתחיל בפרטים הבסיסיים', sub: 'הפרטים האלה יופיעו בכרטיס הציבורי, חוץ מהח״פ, שמשמש לאימות בלבד.' },
  ...
  { key: 'medical', name: 'אחריות מקצועית', title: 'מי אחראי מקצועית?', sub: 'זה הפרט הראשון שלקוחות בודקות, והוא מבדיל בין אינדקס אמין לסתם רשימה.' },
  { key: 'services', name: 'טיפולים ומחירים', title: 'טיפולים ומחירים', sub: 'שלושה טיפולים מספיקים לפרסום. אפשר להוסיף ולערוך אותם בכל עת בלוח הבקרה.' },
  { key: 'hours', name: 'שעות פעילות', title: 'מתי אתם פתוחים?', sub: 'כברירת מחדל: ראשון עד שישי פתוח, שבת סגור. אפשר לשנות כל יום בנפרד.' },
  { key: 'photos', name: 'תמונות', title: 'תמונות הכרטיס', sub: 'תמונת כיסוי, לוגו וגלריה. אפשר לדלג ולהשלים אחר כך, אבל תמונות משפיעות ישירות על מספר הפניות.' },
  { key: 'verify', name: 'סיכום ואימות', title: 'בדיקה אחרונה ושליחה', sub: 'עברו על הפרטים ואשרו את ההצהרות, ואנחנו נתחיל באימות.' },
] as const;
...
export const DECLARATIONS: Array<{ key: DeclKey; label: string; note: string }> = [
  ...
  { key: 'medical', label: 'טיפולים רפואיים בקליניקה מבוצעים על ידי רופא/ה בלבד', note: 'חובה כשנבחרו קטגוריות של הזרקות' },
];
...
export function validate(p: Omit<JoinPayload, 'plan'>) {
  ...
  const errors: Record<StepKey, string> = {
    basics: !nameOk ? 'נדרש שם מסחרי' : !hpOk ? 'מספר ח״פ או עוסק מורשה צריך להכיל 9 ספרות' : !phoneOk ? 'מספר הטלפון אינו תקין'
    ...
    hours: 'בדקו את שעות הפעילות: שעות בפורמט 09:00, ושעת הסגירה אחרי שעת הפתיחה',
    photos: '',
